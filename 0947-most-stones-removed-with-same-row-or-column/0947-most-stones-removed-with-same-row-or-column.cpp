...
public:
    ...
    void dfs(vector<int>&visited ,vector<vector<int>>& stones , int index){
        ...
        for(int i = 0; i<stones.size() ; i++){
            //if not visited ans are in the same group call dfs
             // passing values of i th stone (non visited) and index th stone
    ...
    int removeStones(vector<vector<int>>& stones) {
     ...
     //  Hence our answer will be total count - the number of the connected components
        ...
        return n- components;//6-1(nonvisited) = 5
    }
};